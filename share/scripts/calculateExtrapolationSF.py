...
'''
#calcExtrapoSF('Bveto','CutBvetoPassISO', 'CutBvetoFailISO' 'LeptonEtaELEFF', 'ehad','LFR',0.5)
# calcExtrapoSF('WFR', 'CutBvetoPassID', 'CutBvetoFailID', 'TauPtFF1p', 'lephad', 'WFR')
...
def calcExtrapoSF(category, datapath, bkgpath1, bkgpath2, nominator, denominator, histogram, channel, region, mcVar=0.1, syslist=[]):
    print '----------------------------------------------------------'
    ...
    ## nominal
    histoObs = reader.getHistogram('{:s}-{:s}'.format(dataPath.format(channel),bkgpath1.format(channel)),'{:s}/{:s}'.format(nominator,histogram))
    histoExp = reader.getHistogram('{:s}'.format(bkgpath2.format(channel,channel,channel)),'{:s}/{:s}'.format(denominator,histogram))
    ...
    # calculate nominal
    FF_histo=histoObs.Clone()
    FF_histo.Divide(histoExp)
    FF_histo.SetName('{:s}ExtrapoSF_{:s}_{:s}'.format(category,channel,histogram))
...
#        # todo
    ...
    file = TFile('ExtrapolationSFs/'+region+category+channel+histogram+'.root','RECREATE')
    file.cd()
    FF_histo.Write()
    FF_histo_up.Write()
    FF_histo_down.Write()
