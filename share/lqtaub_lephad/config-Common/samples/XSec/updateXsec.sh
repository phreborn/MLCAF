...
OUTPUT_FILE="${CAFANALYSISSHARE:?}/lqtaub_lephad/config-Common/samples/XSec/XS.csv-$(date +"%Y-%m-%d")"

echo -e "\e[93mUpdating Xsec...\e[39m"

# collect DSIDs from the map file, including commented samples
DSIDS=$(grep -F '/$(channel)/' "${CAFANALYSISSHARE:?}/lqtaub_lephad/config-Common/samples/maps/map.map" | cut -d ' ' -f 1 | tr -d '#[:blank:]')
