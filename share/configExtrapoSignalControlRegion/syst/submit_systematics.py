...
#some configurations
s_queue='medium'
s_nominal_dir='batchOutput/unmerged_SRreweight'
s_config_path='configExtrapoSignalControlRegion/htautau_lephad_sr_reweight.cfg'
s_config_path_mc16c='configExtrapoSignalControlRegion/htautau_lephad_sr_reweight_mc16c.cfg'
#s_config_path='ConfigSignalRegion/htautau_lephad_sr_reweightcontid.cfg'
#s_sys_file_path='/dybfs/juno/xiaozhong/hanfei/mc/mc16a/sys/'
s_sys_file_path='/dybfs/juno/xiaozhong/hanfei/mc/mc16d/sys/'
#s_sample_prefix='samples-htautau_lephad_sr_reweight'
#s_sample_prefix='samples-htautau_lephad_sr_reweightcontid'
...
l_extraposfs=[
["extraposf","ExtrapolationSFs_Btag1p_1down"],
["extraposf","ExtrapolationSFs_Btag1p_1up"],
["extraposf","ExtrapolationSFs_Btag3p_1down"],
["extraposf","ExtrapolationSFs_Btag3p_1up"],
...
if __name__=='__main__':
  if b_submit:
    if b_makeSamples:
      ...
      if args.systype=='weightvar':
        command1="prepare.py {:s} --options mcPaths='{:s}:NOMINAL' outputFile='sampleFolders/prepared/samples-prepared-htautau_lephad_sr_reweight_mc16c-NOMINAL.root'".format(s_config_path_mc16c,s_sys_file_path)
        print(command1)
        if not args.dry: os.system(command1)
        command2="initialize.py {:s} --options mcPaths='{:s}:NOMINAL' inputFile='sampleFolders/prepared/samples-prepared-htautau_lephad_sr_reweight_mc16c-NOMINAL.root' outputFile='sampleFolders/initialized/samples-initialized-htautau_lephad_sr_reweight_mc16c-NOMINAL.root'".format(s_config_path_mc16c,s_sys_file_path)
        print(command2)
    ...
    for systematic in l_systematics:
      ...
      if b_makeSamples:
        if option=='treevariation':
          ...
          command1="prepare.py {:s} --options mcPaths='{:s}:{:s}' outputFile='sampleFolders/prepared/samples-prepared-htautau_lephad_sr_reweight_mc16c-{:s}.root'".format(s_config_path_mc16c,s_sys_file_path,sys,sys)
          print(command1)
          if not args.dry: os.system(command1)
          command2="initialize.py {:s} --options mcPaths='{:s}:{:s}' inputFile='sampleFolders/prepared/samples-prepared-htautau_lephad_sr_reweight_mc16c-{:s}.root' outputFile='sampleFolders/initialized/samples-initialized-htautau_lephad_sr_reweight_mc16c-{:s}.root'".format(s_config_path_mc16c,s_sys_file_path,sys,sys,sys)
          print(command2)
          if not args.dry: os.system(command2)
        # if making sample files: continue, because we don't want to submit jobs before we have inputs ready;
        continue
      ...
      print('mkdir batchOutput/unmerged_SRreweightSys_{:s}'.format(sys))
      if not args.dry: os.system('mkdir batchOutput/unmerged_SRreweightSys_{:s}'.format(sys))
      ...
      for files in l_files:
        ...
        for file in l_file:
          print('ln -sv ../../{:s} batchOutput/unmerged_SRreweightSys_{:s}'.format(file,sys))
          if not args.dry: os.system('ln -sv ../../{:s} batchOutput/unmerged_SRreweightSys_{:s}'.format(file,sys))
      ...
      if option=='treevariation': # if tree variation, we need to provide different input file:
        temp_option="inputFile='sampleFolders/initialized/samples-initialized-htautau_lephad_sr_reweight_mc16c-{:s}.root'".format(sys)
      elif option=='weightvar': # if sf variation, use nominal branch but sys samples:
        temp_option="inputFile='sampleFolders/initialized/samples-initialized-htautau_lephad_sr_reweight_mc16c-NOMINAL.root' aliases.{:s}={:s} {:s}={:s} aliases.weightvariation={:s} weightvariation={:s}".format(option,sys,option,sys,sys,sys)
      ...
      jobs_file = ""
      if args.systype == "fakevar" or option=='extraposf':
      ...
      command='submit.py {:s} --jobs configExtrapoSignalControlRegion/syst/{:s} --identifier SRreweightSys_{:s} --allowArgChanges --submit condor --options {:s}'.format(s_config_path,jobs_file,sys,temp_option)
      print(command)
  ...
  if b_merge and not b_submit:
    ...
    for systematic in l_systematics:
      ...
      command='tqmerge -o sampleFolders/analyzed/samples-analyzed-htautau_lephad_sr_reweight-{:s}.root -t analyze batchOutput/unmerged_SRreweightSys_{:s}/*.root'.format(sys,sys)
      print(command)
      if not args.dry: os.system(command)
