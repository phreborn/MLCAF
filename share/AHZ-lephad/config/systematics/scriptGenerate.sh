...
for CHANNEL in "ehad" "muhad"; do
    ...
    python "${CAFANALYSISBASE:?}/tools/generateSystematicsBands.py" \
        --input "sampleFolders/analyzed/samples-analyzed-bsmtautau_lephad_sr_sys.root" \
        --systematics "${CAFANALYSISSHARE:?}/bsmtautau_lephad/config-SignalControlRegion/syst/lists/all.tq" \
