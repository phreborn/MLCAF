...
def WriteCondorScripts(StoreString,Type):
  ...
  exlines = []
  if 'SYS' != Type:
    region = StoreString.split('-')[0]
    print(region)
    exlines.append('\nsource /afs/cern.ch/work/h/hye/Leptoquark/AnalysisFramework/BSMtautauCAF/share/LQtaub-lephad/config/{}/analyze-merge-{}.sh'.format(region,StoreString))
  elif 'SYS' == Type:
    exlines.append('\nsource /afs/cern.ch/work/h/hye/Leptoquark/AnalysisFramework/BSMtautauCAF/share/LQtaub-lephad/config/SYS/analyze-merge-FF.sh {}'.format(StoreString))
