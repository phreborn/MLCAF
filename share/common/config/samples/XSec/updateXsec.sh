...
OUTPUT_FILE="${CAFANALYSISSHARE:?}/common/config/samples/XSec/XS.csv-$(date +"%Y-%m-%d")"

echo -e "\e[93mUpdating Xsec...\e[39m"

# collect DSIDs from the map file, including commented samples
DSIDS=$(grep -F '/$(channel)/' "${CAFANALYSISSHARE:?}/common/config/samples/maps/default.map" | cut -d ' ' -f 1 | tr -d '#[:blank:]')
