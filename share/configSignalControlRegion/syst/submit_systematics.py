...
def create_cmd_log(option, sys, stage):
  ...
  if stage == 'prepare':
    ...
    s_sys_file_path_hack = s_sys_file_path.replace('sys','sys_{:s}'.format(sys)) 
    cmd="initialize.py {:s} --options campaignsConfig='{:s}'  inputFile='sampleFolders/prepared/samples-prepared-htautau_lephad_sr-{:s}.root' outputFile='sampleFolders/initialized/samples-initialized-htautau_lephad_sr-{:s}.root'".format(s_config_path,s_sys_file_path_hack,sys,sys)
  #######################
  # Analyze samples
  #######################
  elif stage == 'analyze':
    ...
    # obtain the command
    cmd='submit.py {:s} --jobs configSignalControlRegion/syst/{:s} --identifier SRsys_{:s} --allowArgChanges --submit condor --maxSampleSize 60000 --maxSampleCount 75 --options {:s}'.format(s_config_path,jobs_file,sys,extra_option)
  #######################
  # Merge samples
  #######################
  elif stage == 'merge':
