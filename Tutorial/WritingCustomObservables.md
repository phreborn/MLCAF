...
There are three main steps that need to be performed to introduce a new observable to your existing analysis
1. Create and write \emph{source code} with actual calculation of desired quantity
2. Make framework aware of new observable with \emph{python snippet}
3. Integrate observable as \emph{histograms/cuts/weights}/\ldots in analysis
...
Once we have confirmed that the wizard should build the observable, two new files are created: `CAFExample/MjjMaxObservable.h' and `Root/MjjMaxObservable.h`. The important parts in the files contain comments that guide you through the further process. There are example blocks given that give very adequate pieces of code that we want to use.
...
In `initializeSelf()` we will initialize the member variable `mContName` by retrieving the desired container name from the sample folder.
```
TString CandName = "";
if(!this->fSample->getTagString("~cand",CandName)) return false;
this->mCandName = "Event"+CandName;
```
Doing this in the initialize function prevents from running time costly string parsing functions for each event. Now we have to implement the actual calculation of the desired quantity that is to be retrieved. Therefore the getValue() function needs to be extended by at least the following lines:
```
  // retrieve candidate
...
Once your class is implemented and compiles fine along with the other observable classes we can go on to the next step.
...
Then, all we need to do is list the path to your script in the [analyze](https://gitlab.cern.ch/atlas-caf/CAFExample/blob/master/share/xAOD/config/master/analyze-xAOD-Example.cfg) config file to have the framework execute your code and add your observable to the database. The relevant part you should add in `analyze-xAOD-Example.cfg` is:
```
# add any observables
