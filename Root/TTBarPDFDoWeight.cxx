#include "BSMtautauCAF/TTBarPDFDoWeight.h"
#include <limits>
...
ClassImp(TTBarPDFDoWeight)



TTBarPDFDoWeight::TTBarPDFDoWeight() {
...
TTBarPDFDoWeight::TTBarPDFDoWeight(const TString& expression) : LepHadObservable(expression) {
  ...
  if ( ! TQTaggable::getGlobalTaggable("aliases")->getTagBoolDefault("ApplyPDFLoWeight", false) ) return;
...
TObjArray* TTBarPDFDoWeight::getBranchNames() const {
...
bool TTBarPDFDoWeight::initializeSelf() {
  if ( ! TQTaggable::getGlobalTaggable("aliases")->getTagBoolDefault("ApplyPDFLoWeight", false) ) return true;
...
bool TTBarPDFDoWeight::finalizeSelf() {
...
double TTBarPDFDoWeight::getValue() const {
  // Check whether we want to apply the fake factor
  bool apply = false;
  if (! TQTaggable::getGlobalTaggable("aliases")->getTagBool("ApplyPDFLoWeight", apply)) {
    ERRORclass("Can not get ApplyPDFLoWeight tag");
  }
  if (!apply) return 1.0;
