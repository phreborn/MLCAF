#include "BSMtautauCAF/TopReweightLQ.h"
#include <limits>
...
ClassImp(TopReweightLQ)

//______________________________________________________________________________________________

TopReweightLQ::TopReweightLQ(){
  // default constructor
...
TObjArray* TopReweightLQ::getBranchNames() const {
...
//______________________________________________________________________________________________
double TopReweightLQ::getValue() const {
  ...
  else if (1<= f_n_bjets) region = "Btag";//delete Btag
  ...
  // parameterization
  TString param = "SumOfPt";//since the o in scalfactor_histo_namet is cmall, use "SumOfPt" here

  TString histName = "LQTCR"+ period + channel + region + param + "SF";
  ...
  ///////////////////////////////////////////////////////////////
  if    ( (fSysName.Contains("TopReweightLQ_LepHadBtag1p_1up")    && f_n_bjets>0) ||
          (fSysName.Contains("TopReweightLQ_LepHadBtag3p_1up")    && f_n_bjets>0 )) {
    retval += retval_error;
  }
  else if((fSysName.Contains("TopReweightLQ_LepHadBtag1p_1down")   && f_n_bjets>0) ||
          (fSysName.Contains("TopReweightLQ_LepHadBtag3p_1down")   && f_n_bjets>0 )) {
    retval -= retval_error;
  }
...
TopReweightLQ::TopReweightLQ(const TString& expression) : LepHadObservable(expression)
{
  ...
  TFile* aFile= TFile::Open("ScaleFactors/LQTCR_SF.root");//need to be modified later
  if (!aFile) {
    std::cout << "ERROR: can not find LQTCR_SF.root " << std::endl;
  }
...
const TString& TopReweightLQ::getExpression() const {
...
bool TopReweightLQ::hasExpression() const {
...
void TopReweightLQ::setExpression(const TString& expr){
  // set the expression to a given string
  this->fExpression = expr;
}
//______________________________________________________________________________________________

bool TopReweightLQ::initializeSelf(){
...
bool TopReweightLQ::finalizeSelf(){
