...
double WjetsReweight::getValue() const {
  ...
  float retval = h_nominal->GetBinContent(h_nominal->FindBin(f_lephad_met_lep1_cos_dphi));
  float retval_up = h_up->GetBinContent(h_up->FindBin(f_lephad_met_lep1_cos_dphi));
  float retval_down = h_down->GetBinContent(h_down->FindBin(f_lephad_met_lep1_cos_dphi));
...
bool WjetsReweight::parseExpression(const TString& expr){
...
void WjetsReweight::clearParsedExpression(){
  // clear the current expression
}

//______________________________________________________________________________________________

TString WjetsReweight::getActiveExpression() const {
