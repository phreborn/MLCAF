#include "BSMtautauCAF/STopPDFUpWeight.h"
#include <limits>
...
ClassImp(STopPDFUpWeight)



STopPDFUpWeight::STopPDFUpWeight() {
...
STopPDFUpWeight::STopPDFUpWeight(const TString& expression) : LepHadObservable(expression) {
  ...
  if ( ! TQTaggable::getGlobalTaggable("aliases")->getTagBoolDefault("ApplySTopPDFHiWeight", false) ) return;
...
TObjArray* STopPDFUpWeight::getBranchNames() const {
...
bool STopPDFUpWeight::initializeSelf() {
  if ( ! TQTaggable::getGlobalTaggable("aliases")->getTagBoolDefault("ApplySTopPDFHiWeight", false) ) return true;
...
bool STopPDFUpWeight::finalizeSelf() {
...
double STopPDFUpWeight::getValue() const {
  // Check whether we want to apply the fake factor
  bool apply = false;
  if (! TQTaggable::getGlobalTaggable("aliases")->getTagBool("ApplySTopPDFHiWeight", apply)) {
    ERRORclass("Can not get ApplySTopPDFHiWeight tag");
  }
  if (!apply) return 1.0;
