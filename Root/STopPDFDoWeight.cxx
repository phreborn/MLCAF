#include "BSMtautauCAF/STopPDFDoWeight.h"
#include <limits>
...
ClassImp(STopPDFDoWeight)



STopPDFDoWeight::STopPDFDoWeight() {
...
STopPDFDoWeight::STopPDFDoWeight(const TString& expression) : LepHadObservable(expression) {
  ...
  if ( ! TQTaggable::getGlobalTaggable("aliases")->getTagBoolDefault("ApplySTopPDFLoWeight", false) ) return;
...
TObjArray* STopPDFDoWeight::getBranchNames() const {
...
bool STopPDFDoWeight::initializeSelf() {
  if ( ! TQTaggable::getGlobalTaggable("aliases")->getTagBoolDefault("ApplySTopPDFLoWeight", false) ) return true;
...
bool STopPDFDoWeight::finalizeSelf() {
...
double STopPDFDoWeight::getValue() const {
  // Check whether we want to apply the fake factor
  bool apply = false;
  if (! TQTaggable::getGlobalTaggable("aliases")->getTagBool("ApplySTopPDFLoWeight", apply)) {
    ERRORclass("Can not get ApplySTopPDFLoWeight tag");
  }
  if (!apply) return 1.0;

  if (isData()) return 1.0;
  if ( !isSTop() ) return 1.0;
